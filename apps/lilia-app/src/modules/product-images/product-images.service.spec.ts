...
describe('ProductImagesService — sync Product.imageUrl', () => {
  ...
  let common: {
    assertRestaurantOwnership: jest.Mock;
    assertUnderMax: jest.Mock;
    demoteOtherCovers: jest.Mock;
    cleanupCloudinary: jest.Mock;
  };
  ...
  beforeEach(async () => {
    ...
    prisma = {
      product: {
        findUnique: jest.fn().mockResolvedValue({ restaurantId: 'resto-1' }),
      },
  ...
  it("create(isCover=true) écrit Product.imageUrl = url de l'image", async () => {
    tx.productImage.create.mockResolvedValue({
      id: 'img-1',
      productId: 'p-1',
      url: 'http://c/cover.jpg',
      isCover: true,
    });
    ...
    expect(tx.product.update).toHaveBeenCalledWith({
      where: { id: 'p-1' },
      data: { imageUrl: 'http://c/cover.jpg' },
    });
  });
  ...
  it('update(isCover=true) synchronise Product.imageUrl', async () => {
    ...
    tx.productImage.update.mockResolvedValue({
      id: 'img-3',
      productId: 'p-1',
      url: 'http://c/new.jpg',
      isCover: true,
  ...
  it('remove(cover) promeut la suivante et met à jour imageUrl', async () => {
    ...
    tx.productImage.findFirst.mockResolvedValue({
      id: 'img-next',
      productId: 'p-1',
      url: 'http://c/next.jpg',
    });
